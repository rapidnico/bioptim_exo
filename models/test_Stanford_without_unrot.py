...
# Load the model
model_name = Models.Stanford_VA_upper_limb_model_0_40_copie.value
...
all_q[6, :] = np.linspace(0, 3.14, n_frames)
q_elevation = all_q[6, :]
...
for i in range(1, len(q_elevation)):
    ...
    all_q[2, i] = q_elevation[i] * (-0.128282/2.61799)  #same as unrot_hum_R2
    all_q[3, i] = q_elevation[i] * (1.03673 / 2.61799) # same as unrot_hum_R3
    all_q[4, i] = q_elevation[i] * (0.46603 / 2.61799)  # same as unrot_hum_R1
...
# #A_accromio_R2, Angles_table_accromioclav_R2, angles_accrmio_R2 = matrice_1_rotation(19, n_frames, model_name, all_q)
